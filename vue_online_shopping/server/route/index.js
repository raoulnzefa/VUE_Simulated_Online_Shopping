    ...
    */
    ...
    function delReg(insUserInfo, res) {
        db.query(insUserInfo, (err) => {
            if (err) {
                console.error(err);
                res.send({ 'msg': 'Register failed', 'status': 0 }).end();
            } else {
                res.send({ 'msg': 'Register succeed', 'status': 1 }).end();
            }
        })
    };
    route.post('/login', (req, res) => {
        ...
        db.query(selectUser, (err, data) => {
            if (err) {
                console.log(err);
                res.send({ 'msg': 'Login failed', 'status': 0 }).end();
            } else {
                if (data.length == 0) {
                    res.send({ 'msg': 'The user do not exist', 'status': -1 }).end();
                } else {
                    let dataw = data[0];
